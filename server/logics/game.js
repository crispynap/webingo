...
const startMsgs = [
  ...
  { event: 'set bank' },
  { event: 'msg', msg: `빈집이라는 주거 공동체들의 전월세 보증금을 모으기 위해서였습니다.` },
  { event: 'msg', msg: `여러분 중에는 빈집에 살고 있는 사람도 있었고,` },
  { event: 'msg', msg: `빈집에 놀러오는 사람이나 그냥 같은 동네에 사는 사람도 있었으며,` },
  { event: 'msg', msg: `빈집은 잘 모르지만 이 은행의 취지에 공감한 사람도 있었습니다.` },
  { event: 'msg', msg: `여러분은 이 은행의 이름을 빈마을 금고라는 의미에서 <b>빈고</b>라 지었습니다.` },
];
    ...
    const emitsQueue = function* (items) {
      for (let item of items) {
        switch (item.event) {
          ...
          default: emit(item.event, item.msg);
        }
